...
- **תהליך:**
  1. בונה טווח זמן עבור יום "אתמול" לפי timezone `Asia/Jerusalem` (כולל DST אוטומטי):
     ...
     - חשוב: הטווח חייב להסתיים ב"אתמול" ולא לכלול "היום" — אחרת ה-cron (שרץ ב-07:00 UTC, כ-09:00-10:00 שעון ישראל) יתפוס גם פרקים שעדיין לא שודרו באותו יום וישלח עליהם התראה מוקדם מדי.
  2. שולף את כל הפרקים המתוזמנים לטווח זה מ-AniList `getAiringScheduleInRange` (רק פרקים שכבר שודרו בפועל, כי הטווח כולו בעבר)
...
- **בנייה של סקשן "צופה":** שליפת כל `TrackedAnime` עם `watchStatus='watching'` עבור המשתמש, enrichment עם כותרה עברית + מספר עונה נוכחי ו-סה"כ עונות בסדרה
- מייל קונסוליד (מיי יחיד) עם 3 סקשנים: "בשידור כעת", "הוכרזה עונה", "צופה"
...
| `sendConsolidatedMonthlyEmail` | עדכון חודשי משולב | מייל עם 3 סקשנים: "בשידור כעת", "הוכרזה עונה", "צופה". **כרטיסיות:** **בקשן "בשידור":** פורמט קומפקטי — כותרה (עברית + עונה אם רלוונטי) + עד 2-3 פרקים קרובים (בפורמט צפוף: "פרק X - תאריך")  + תמונה `90px` בצד ימין (flex-direction:row-reverse). כרטיס בגובה מינימום `110px` עם דוד ורוד (`rgba(224,23,107,0.15)`). **בקשן "הוכרזה":** תמונה `90px` בצד שמאל + תוכן בצד ימין (flex). כותרה (עברית + עונה אם רלוונטי) + מספר עונות קיימות (אם קיים) + מספר פרקים קיימים (אם קיים) + תאריך הכרזה (צהוב). דוד צהוב (`rgba(251,191,36,0.15)`). **בקשן "צופה":** תמונה `90px` בצד שמאל + כותרה (עברית) + מספר עונה נוכחי (אופציונלי) + סה"כ עונות בסדרה (אופציונלי). דוד כחול (`rgba(33,150,176,0.15)`). סקשן זה משלב את כל `TrackedAnime` עם `watchStatus='watching'` עבור המשתמש. **תמונות:** `<img>` עם `width="90"` + `style="width:90px;height:100%;object-fit:cover;display:block;"` — תאימות מלאה לקליינטי מייל. placeholder: `<div>` בצבע background כהה. **inline attachments:** כל תמונה מורדת server-side וצמודה כ-inline attachment עם CID ייחודי — `<img src="cid:...">` במקום URL חיצוני. |
| `sendConsolidatedMonthlyEmail` — כותרת וכותרת דף | קבועה | כותרת מייל: `עדכון חודשי - ANIME TRACKER 🎌`. כותרת דף (בגוף המייל): "עדכון **חודשי**" ללא תאריך (הוסרה שורה שהציגה חודש ושנה בעברית) |
| `sendNewEpisodeEmail` | כשיוצאים פרקים חדשים באותו יום לאנימות במעקב (דרך `/api/check-episode-releases`) | מייל עם רשימת הפרקים שיוצאים היום וטבלת הפרקים הקרובים הבאים. כותרת קבועה: `פרקים חדשים להיום - ANIME TRACKER` |
