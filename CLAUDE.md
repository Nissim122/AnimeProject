...
| `sendConsolidatedMonthlyEmail` | עדכון חודשי משולב | מייל עם 3 סקשנים: "בשידור כעת", "הוכרזה עונה", "ממתין לצפייה". **כרטיסיות — עיצוב אחיד לכל 3 הסקשנים:** עמודת תמונה `76px×107px` בצד שמאל + תוכן בצד ימין (flex). **תמונות:** `<img>` עם `width="76" height="107"` + `style="width:76px;height:107px;object-fit:cover;display:block;"` — תאימות מלאה לקליינטי מייל (Gmail/Outlook/Apple Mail). **עמודת תמונה:** `align-self:flex-start` (לא מתמתחת עם גוף הכרטיס). placeholder: `<div>` בגובה `107px`, background כהה. **בקשן "בשידור":** תמונה + badge + כותרה (עברית בלבד). עד 3 פרקים קרובים עם כותרת "פרקים קרובים" וכרטיסייה לכל פרק (מספר + תאריך). **בקשן "הוכרזה":** תמונה + כותרה (עברית בלבד) + עונה + מספר עונות קיימות + תאריך הכרזה (full format: יום/חודש/שנה) ב-container צהוב + expected episodes. **בקשן "ממתין":** תמונה + כותרה (עברית בלבד) + זמינות (✓ ירוק). **ב-mobile (≤480px):** כרטיס עובר לפורמט עמוד (flex-direction:column), תמונה full-width בגובה `170px` עם `object-fit:cover`. |
