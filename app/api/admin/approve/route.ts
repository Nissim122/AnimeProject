...
export async function GET(req: NextRequest) {
  ...
  if (approval.status === 'APPROVED') {
    return htmlPage(
      'כבר מאושר',
      `<div class="icon">✅</div><h1>כבר מאושר</h1><p>המשתמש <strong style="color:#d1ddf9">${approval.email}</strong> כבר קיבל גישה.</p>`,
      '#4ade80'
    )
  }
  ...
  return htmlPage(
    'אושר בהצלחה',
    `<div class="icon">✅</div><h1>גישה אושרה!</h1><p>המשתמש <strong style="color:#d1ddf9">${approval.email}</strong> קיבל גישה מלאה למערכת.<br><br>נשלחה לו הודעת אישור במייל.</p>`,
    '#4ade80'
  )
}
