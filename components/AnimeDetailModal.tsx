...
export default function AnimeDetailModal({ anime, trackedIds, onTrack, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/seasons?id=${anime.id}`)
      .then((r) => r.json())
      .then((data) => {
        const list: AnimeResult[] = data.seasons ?? []
        setSeasons(list)
      ...
      >
        ...
        <div className="overflow-y-auto flex-1 p-4">
          {loading ? (
            ...
            <p className="text-gray-400 text-xs text-right mb-3">
              לחץ על עונה לבחירה
            </p>
          )}
        ...
        <div className="px-5 py-4 border-t border-gray-700 flex items-center justify-between gap-3">
          ...
          <button
            onClick={handleTrack}
            disabled={loading || !selectedAnime || alreadyTracked}
